...
#define JNI_METHOD(return_type, method_name) \
  JNIEXPORT return_type JNICALL              \
      Java_org_echoline_quake2_MainActivity_##method_name
...
JNI_METHOD(void, nativeMainLoop)
(JNIEnv *env, jobject obj) {
    char *args[] = { "quake2", NULL };
    Qcommon_Run(1, args);
}

JNI_METHOD(void, nativeSurfaceChanged)
(JNIEnv *env, jobject obj, jobject surface) {
    if (surface != NULL)
        aNativeWindow = ANativeWindow_fromSurface(env, surface);
    else if (aNativeWindow != NULL) {
