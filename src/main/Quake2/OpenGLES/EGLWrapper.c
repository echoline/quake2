...
extern struct ANativeWindow *aNativeWindow;
...
static EGLNativeWindowType eglwGetNativeWindow()
{
	...
	uint32_t screenWidth, screenHeight;
 	graphics_get_display_size(0, &screenWidth, &screenHeight);
...
bool eglwInitialize(EglwConfigInfo *minimalCfgi, EglwConfigInfo *requestedCfgi, bool maxQualityFlag) {
    ...
    // Create an EGL window surface.
    {
        ...
        eglw->surface = eglCreatePbufferSurface(eglw->display, eglw->config, SURFACE_ATTRIBUTES);
